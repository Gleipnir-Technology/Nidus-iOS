...
actor MediaLibrary {
	...
	func save(photo: Photo) async throws {
		...
		try await performChange {
			...
			creationRequest.addResource(
				with: photo.isProxy ? .photoProxy : .photo,
				data: photo.data,
				options: options
			)
	...
	private func performChange(_ change: @Sendable @escaping () -> PHObjectPlaceholder?)
		async throws
	{
	...
	private var currentLocation: CLLocation? {
		get async throws {
			...
			return try await CLLocationUpdate.liveUpdates().first(where: { _ in true })?
				.location
		}
	}
}
