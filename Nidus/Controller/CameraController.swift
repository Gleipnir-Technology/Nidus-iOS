...
final class CameraController: Camera {
	...
	/// Callbacks when the picture is save
	private var pictureSavedCallbacks: [(Photo) -> Void] = []
	...
	func onPictureSave(_ callback: @escaping (Photo) -> Void) {
		pictureSavedCallbacks.append(callback)
	}

	private func handlePictureSave(_ picture: Photo) {
		for c in pictureSavedCallbacks {
			c(picture)
		}
	}
	...
	private func observeState() {
		...
		Task {
			...
			for await isShowingFullscreenControls
				in await captureService.$isShowingFullscreenControls.values
			{
				withAnimation {
