...
actor CaptureService {
	...
	private let sessionQueue = DispatchSerialQueue(
		label: "com.example.apple-samplecode.AVCam.sessionQueue"
	)
	...
	func createControls(for device: AVCaptureDevice) -> [AVCaptureControl] {
		...
		guard let controls = controlsMap[device.uniqueID] else {
			...
			if device.isLockingFocusWithCustomLensPositionSupported {
				...
				let lensSlider = AVCaptureSlider(
					"Lens Position",
				...
				lensSlider.setActionQueue(sessionQueue) { lensPosition in
					do {
						...
						logger.info(
							"Unable to change the lens position: \(error)"
						)
					}
				}
		...
		Task {
			...
			for await camera in systemPreferredCamera.changes {
				...
				if let camera, currentDevice != camera {
					logger.debug(
						"Switching camera selection to the system-preferred camera."
					)
	...
	private var videoPreviewLayer: AVCaptureVideoPreviewLayer {
		...
		guard
			let previewLayer = captureSession.connections.compactMap({
				$0.videoPreviewLayer
			}).first
		else {
			fatalError(
	...
	private func focusAndExpose(at devicePoint: CGPoint, isUserInitiated: Bool) throws {
		...
		let focusMode =
			isUserInitiated ? AVCaptureDevice.FocusMode.autoFocus : .continuousAutoFocus
		...
		let exposureMode =
			isUserInitiated
			? AVCaptureDevice.ExposureMode.autoExpose : .continuousAutoExposure
	...
	func setHDRVideoEnabled(_ isEnabled: Bool) {
		...
		do {
			...
			logger.error(
				"Unable to obtain lock on device and can't enable HDR video capture."
			)
		}
	}
	...
	private func observeNotifications() {
		Task {
			...
			.compactMap({
				$0.userInfo?[AVCaptureSessionInterruptionReasonKey] as AnyObject?
			})
			.compactMap({
				AVCaptureSession.InterruptionReason(rawValue: $0.integerValue)
			}) {
