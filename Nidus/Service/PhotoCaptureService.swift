...
//  Nidus Notes
//
//  Created by Eli Ribble on 8/13/25.
//
...
final class PhotoCapture: OutputService {
	...
	private func createPhotoSettings(with features: PhotoFeatures) -> AVCapturePhotoSettings {
		...
		photoSettings.livePhotoMovieFileURL =
			features.isLivePhotoEnabled ? URL.movieFileURL : nil
	...
	private func monitorProgress(
		of delegate: PhotoCaptureDelegate,
			...
			for await activity in delegate.activityStream {
				...
				if activity.isLivePhoto != isLivePhoto {
					...
					if livePhotoCount > 1 {
						...
						currentActivity = .photoCapture(
							willCapture: activity.willCapture,
							isLivePhoto: true
						)
					}
				}
	...
	func updateConfiguration(for device: AVCaptureDevice) {
		...
		photoOutput.maxPhotoDimensions =
			device.activeFormat.supportedMaxPhotoDimensions.last ?? .zero
...
private class PhotoCaptureDelegate: NSObject, AVCapturePhotoCaptureDelegate {
	...
	init(continuation: PhotoContinuation) {
		...
		let (activityStream, activityContinuation) = AsyncStream.makeStream(
			of: CaptureActivity.self
		)
	...
	func photoOutput(
		...
		activityContinuation.yield(
			.photoCapture(willCapture: true, isLivePhoto: isLivePhoto)
		)
	}
	...
	func photoOutput(
		...
		if let error {
			logger.debug(
				"Error processing Live Photo companion movie: \(String(describing: error))"
			)
		}
	...
	func photoOutput(
		_ output: AVCapturePhotoOutput,
		didFinishCapturingDeferredPhotoProxy deferredPhotoProxy:
